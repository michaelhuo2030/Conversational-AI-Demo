...
class HomeContentViewCell: UICollectionViewCell {
    ...
    func setupData(model: HomeContentModel?) {
        ...
        let attrs = NSMutableAttributedString(string: content, attributes: [.foregroundColor: UIColor(hex: "#000000", alpha: 1.0) ?? .black,
                                                                            .font: UIFont.systemFont(ofSize: 16, weight: .medium),
                                                                            .paragraphStyle: paragraphStyle])
        attrs.addAttributes([.foregroundColor: UIColor(hex: "#303553", alpha: 1.0) ?? .white,
                             .font: UIFont.systemFont(ofSize: 10)], range: NSRange(location: (model?.title?.count ?? 0) + 1,
    ...
    private func setupUI() {
        ...
        titleLabel.widthAnchor.constraint(equalToConstant: 77).isActive = true
    }
}
